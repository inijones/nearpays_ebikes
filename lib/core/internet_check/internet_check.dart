...
class InternetCheck {
  ...
  Future<bool> internetConnectionHandler(BuildContext context,
      {bool fromPlayer = false}) async {
    bool internetAccess = await checkInternetAccess();

    if (!internetAccess) {
      if (movedToIntenetScreen == 0) {
        WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
          AppSnackbar(context, isError: true).showToast(text: "text");
        });
